...
namespace Eduardo.OpenAISmartTest.Commands
{
    ...
    internal sealed class TerminalWindowCommand
    {
        ...
        public async System.Threading.Tasks.Task RequestToWindowAsync(string command)
        {
            if (window == null)
            {
                throw new Exception("Por favor, abra a janela de ferramentas primeiro.");
            }
        ...
        private static async System.Threading.Tasks.Task InitializeToolWindowAsync(AsyncPackage package)
        {
            ...
            window.SetTerminalWindowProperties(((OpenAISmartTestPackage)package).OptionsGeneral, package);
        }
    }
}
