    ...
    public partial class TerminalWindowControl : UserControl
    {
        ...
        public async void SendRequest(Object sender, ExecutedRoutedEventArgs e)
        {
            try
            {
                ...
                await Claude.RequestAsync(options, selection, ResultHandler);
            }
            catch (Exception ex)
            {
        ...
        public async System.Threading.Tasks.Task RequestToWindowAsync(string command)
        {
            try
            {
                ...
                await VS.StatusBar.ShowProgressAsync("Requesting Claude", 1, 2);

                txtRequest.Text = command;

                txtResponse.Text = string.Empty;

                await Claude.RequestAsync(options, command, ResultHandler);
            }
            catch (Exception ex)
            {
                await VS.StatusBar.ShowProgressAsync(ex.Message, 2, 2);
